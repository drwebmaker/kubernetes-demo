...
interface AsyncResultCallback<T, E> {
  (err?: E, result?: T): void;
}
...
export function runShellCommand (command: string, options: any, cb: AsyncResultCallback<ExecOutputReturnValue | ChildProcess | string, string>): void {
  ...
  async.retry({
    ...
    const {code, stderr, stdout} = (result as ExecOutputReturnValue);

    if (process.env.DEBUG === 'true') {
      logger.info(`[CODE=${code}] RESULT COMMAND: \nstdout: ${stdout ? stdout : `${stdout}\n`}stderr: ${stderr ? stderr : `${stderr}\n`}error: ${error ? error : `${error}\n`}`);
    }
...
function isStepShouldBeSkipped (result: string): any {
  const skipMarkers = [ 'already exists', 'scaled', 'AlreadyExists', 'is already in use', 'code=404', 'was not found', 'is not a valid name', 'RUNNING', 'Google Cloud SDK', 'Docker version' ];
...
export function checkDockerTool (externalContext: any, cb: Function): void {
...
export function checkGcloudTool (externalContext: any, cb: Function): void {
  const command = `gcloud -v`;
  const options: ExecOptions = { silent: true };
